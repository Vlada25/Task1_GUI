...
namespace Task1_GUI
{
    ...
    public partial class MainForm : Form
    {
        ...
        public void readData(ref bool isDataCorrect, int combLen, string[] comb, List<int> nomOfCards, TextBox textBox, List<string> colors, List<int> priotityOfColors)
        {
            const int nomPosition = 0, colorPosition = 2;
            for (int i = 0; i < combLen; i++)
            {
                int nom;
                bool result1 = int.TryParse(Convert.ToString(comb[i][nomPosition]), out nom);
                if (!result1)
                {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (!Enum.TryParse(Convert.ToString(c), out color))
                        {
                            Console.WriteLine($"Invalid value: {c}");
                            return;
                        }
                        colors.Add(GetDisplayName(color));
                        priotityOfColors.Add((int)color);
                    }
                }
            }
        }
        public void findHighCard(ref int highCard, ref int n_highCard, int combLen, List<int> nomOfCards, List<string> colors, List<int> priotityOfColors)
        {
            for (int i = 1; i < combLen; i++)
            {
                ...
                {
                    Colors color;
                    int ind1 = priotityOfColors[i];
                    int ind2 = priotityOfColors[n_highCard];
        ...
        public void mainFunc(int combLen1, int combLen2)
        {
            ...
            List<int> priorityOfColors1 = new List<int>();
            List<int> priorityOfColors2 = new List<int>();
            ...
            readData(ref isDataCorrect, combLen1, comb1, nomOfCards1, textBox1, colors1, priorityOfColors1);
            readData(ref isDataCorrect, combLen2, comb2, nomOfCards2, textBox2, colors2, priorityOfColors2);
            if (isDataCorrect == true)
            {
                highCard1 = nomOfCards1[0];
                n_highCard1 = 0;
                findHighCard(ref highCard1, ref n_highCard1, combLen1, nomOfCards1, colors1, priorityOfColors1);
                highCard2 = nomOfCards2[0];
                n_highCard2 = 0;
                findHighCard(ref highCard2, ref n_highCard2, combLen2, nomOfCards2, colors2, priorityOfColors2);
                ...
                {
                    if (priorityOfColors1[n_highCard1] < priorityOfColors2[n_highCard2])
                    {
                        numOfWinner = 1;
                        colorOfWinner = colors1[n_highCard1];
                        nominalOfWinner = nomOfCards1[n_highCard1];
        ...
        private void do_btn_Click(object sender, EventArgs e)
        {
            ...
            bool isSizeInt = true;
            bool result1 = int.TryParse(textBox1.Lines[0], out combLen1);
            bool result2 = int.TryParse(textBox2.Lines[0], out combLen2);
            if (!result1)
            {
